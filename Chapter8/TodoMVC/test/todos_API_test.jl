...
const API_URL = "http://localhost:$APP_PORT/api/v1/todos"
...
@testset "TodoMVC REST API tests" begin
  ...
  @testset "No todos by default" begin
    response = HTTP.get(API_URL, DEFAULT_HEADERS)
  ...
  @testset "Todo creation" begin
    ...
    @testset "Incorrect content-type should fail todo creation" begin
      response = HTTP.post(API_URL, ["Content-Type" => "text/plain", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "Buy milk")); status_exception = false)
    ...
    @testset "Invalid JSON should fail todo creation" begin
      response = HTTP.post(API_URL, ["Content-Type" => "application/json", DEFAULT_HEADERS...], "Surrender your data!"; status_exception = false)
      @test response.status == Genie.Router.BAD_REQUEST
      @test JSON3.read(String(response.body)) == "Invalid JSON message received"
    end

    @testset "Valid JSON with invalid data should fail todo creation" begin
      response = HTTP.post(API_URL, ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "", "completed" => true)); status_exception = false)
      @test response.status == Genie.Router.BAD_REQUEST
    ...
    @testset "Valid payload should create todo" begin
      response = HTTP.post(API_URL, ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "Buy milk")))
    ...
    @testset "One todo should be created" begin
      ...
      response = HTTP.get("$API_URL/1", DEFAULT_HEADERS)
      @test response.status == Genie.Router.OK
  ...
  @testset "Todo updating" begin
    ...
    @testset "Incorrect content-type should fail todo update" begin
      response = HTTP.patch("$API_URL/1", ["Content-Type" => "text/plain", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "Buy soy milk")); status_exception = false)
      @test response.status == Genie.Router.BAD_REQUEST
      @test JSON3.read(String(response.body)) == "Invalid JSON message received"
    end

    @testset "Invalid JSON should fail todo update" begin
      response = HTTP.patch("$API_URL/1", ["Content-Type" => "application/json", DEFAULT_HEADERS...], "Surrender your data!"; status_exception = false)
      @test response.status == Genie.Router.BAD_REQUEST
      @test JSON3.read(String(response.body)) == "Invalid JSON message received"
    end

    @testset "Valid JSON with invalid data should fail todo update" begin
      response = HTTP.patch("$API_URL/1", ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "", "completed" => true)); status_exception = false)
      @test response.status == Genie.Router.BAD_REQUEST
      @test JSON3.read(String(response.body)) == "Todo should not be empty"
    end
    ...
    @testset "Valid payload should update todo" begin
      response = HTTP.patch("$API_URL/1", ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "Buy vegan milk")))
      @test response.status == Genie.Router.OK
    ...
    @testset "Updating a non existing todo should fail" begin
      response = HTTP.patch("$API_URL/100", ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(Dict("todo" => "Buy apples")); status_exception = false)
  ...
  @testset "Todo deletion" begin
    ...
    @testset "Deleting a non existing todo should fail" begin
      response = HTTP.delete("$API_URL/100", ["Content-Type" => "application/json", DEFAULT_HEADERS...]; status_exception = false)
      @test response.status == Genie.Router.NOT_FOUND
      @test JSON3.read(String(response.body)) == "Todo not found"
    end

    @testset "One existing todo should be deleted" begin
      response = HTTP.delete("$API_URL/1", DEFAULT_HEADERS)
      @test response.status == Genie.Router.OK
      @test JSON3.read(String(response.body))["todo"] == "Buy vegan milk"
  ...
  @testset "Todos pagination" begin
    ...
    for todo in todo_list
      response = HTTP.post(API_URL, ["Content-Type" => "application/json", DEFAULT_HEADERS...], JSON3.write(todo))
    end
    ...
    @testset "One per page" begin
      ...
      for page in 1:length(todo_list)
        response = HTTP.get("$API_URL?page=$(page)&limit=1", DEFAULT_HEADERS)
        todos = JSON3.read(String(response.body))
        @test length(todos) == 1
    ...
    @testset "Two per page" begin
      response = HTTP.get("$API_URL?page=1&limit=2", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 2
      @test todos[1]["todo"] == todo_list[1]["todo"]
      @test todos[2]["todo"] == todo_list[2]["todo"]

      response = HTTP.get("$API_URL?page=2&limit=2", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 2
    ...
    @testset "Three per page" begin
      response = HTTP.get("$API_URL?page=1&limit=3", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 3
      @test todos[1]["todo"] == todo_list[1]["todo"]
      @test todos[2]["todo"] == todo_list[2]["todo"]
      ...
      response = HTTP.get("$API_URL?page=2&limit=3", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 1
    ...
    @testset "Four per page" begin
      response = HTTP.get("$API_URL?page=1&limit=4", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 4
      @test todos[1]["todo"] == todo_list[1]["todo"]
      @test todos[2]["todo"] == todo_list[2]["todo"]
      ...
      response = HTTP.get("$API_URL?page=2&limit=4", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test isempty(todos) == true
    end

    @testset "Five per page" begin
      response = HTTP.get("$API_URL?page=1&limit=5", DEFAULT_HEADERS)
      todos = JSON3.read(String(response.body))
      @test length(todos) == 4
      @test todos[1]["todo"] == todo_list[1]["todo"]
      @test todos[2]["todo"] == todo_list[2]["todo"]
